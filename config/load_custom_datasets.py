...
def tokenize_dialog(dialog, tokenizer):
    prompt_tokens = [tokenizer.encode(f"{tokenizer.bos_token}{B_INST} {(prompt['content']).strip()} {E_INST}",
...
def get_datasets(dataset_config, tokenizer, split):
    dataset = datasets.load_dataset(dataset_config.file, split=split)
    ...
    dataset = dataset.map(lambda x: {"thread": get_threads_from_root(x["message_id"])},
                          remove_columns=list(dataset.features))
